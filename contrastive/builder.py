...
class ContrastiveBuilder(builders.ActorLearnerBuilder):
  ...
  def make_replay_tables(
    ...
    # Keep original calculations - we're chunking continuous episode into trajectories
    ...
    # Ensure error_buffer meets reverb's requirement: >= 2 * max(1.0, samples_per_insert)
    min_error_buffer = 2 * max(1.0, self._config.samples_per_insert)
    error_buffer = max(error_buffer, min_error_buffer)
  ...
  def make_adder(self,
    ...
    # Use original max_episode_steps as trajectory length
    # This will chunk the continuous episode into fixed-length trajectories
